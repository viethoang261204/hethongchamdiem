...
export default function AdminCompetitions() {
  ...
  const { data, loading, error, reload, setData } = useApiLoader(() => api.getCompetitions(), []);
  const list = data || [];
  ...
  const filtered = useMemo(() => {
    ...
    if (filterActive === 'active') l = l.filter(c => c.is_active !== false);
  ...
  const openEdit = (c) => {
    setModal({ id: c.id, ...c });
    setForm({ name: c.name, description: c.description || '', location: c.location || '', startDate: c.start_date || '', endDate: c.end_date || '', isActive: c.is_active !== false });
  };
          ...
          <table>
            ...
            <tbody>
              ...
              <tr key={c.id}>
                ...
                <td>{c.is_active !== false ? 'Đang mở' : 'Đã đóng'}</td>
                <td>
